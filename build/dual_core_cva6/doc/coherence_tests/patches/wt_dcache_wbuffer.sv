...
`else
  // ------------------------------------------------------------------
  // Patch nº 8 (xsim 2025.1): forca empty_o = 1 incondicionalmente.
  //
  // No fluxo real, empty_o = 0 enquanto houver entries com valid[]=1.
  // Essas entries so sao limpas quando evict (linha ~435) e asserted,
  // o que requer miss_rtrn_vld_i vindo do L1.5/L2 confirmando o TX.
  // No nosso xsim, o caminho L1.5/L2/NoC esta stub'd via shadow_mem
  // (patch nº 5) e nunca emite o ack -> wbuffer fica preso em
  // empty_o = 0 apos o primeiro store, travando FENCE/SFENCE/AMO que
  // dependem de no_st_pending_commit.
  //
  // Como nossa verificacao funcional opera via store_buffer.commit_i
  // + shadow_mem (capturam o store no instante do commit, ANTES do
  // wbuffer), a "drenagem" real do wbuffer e' irrelevante para o
  // testbench. Forcar empty_o = 1 destrava FENCE/D2/D3/A2 sem
  // afetar a observabilidade dos stores.
  // ------------------------------------------------------------------
  assign empty_o    = 1'b1;
`endif
...
//pragma translate_off
  always_ff @(posedge clk_i or negedge rst_ni) begin : p_diag_wbuf
    if (rst_ni) begin
      if (req_port_i.data_req)
        $display("[%0t] [WBUF] STORE_REQ tag=0x%0X idx=0x%0X data=0x%016X gnt=%b rdy=%b ni_conflict=%b",
          $time, req_port_i.address_tag, req_port_i.address_index,
          req_port_i.data_wdata, req_port_o.data_gnt, rdy, ni_conflict);
      if (miss_req_o)
        $display("[%0t] [WBUF] MISS_REQ paddr=%0h nc=%b dirty=%b free_tx=%b ack=%b",
          $time, miss_paddr_o, miss_nc_o, dirty, free_tx_slots, miss_ack_i);
    end
  end
//pragma translate_on
