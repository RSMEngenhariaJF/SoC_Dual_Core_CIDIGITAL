  ...
  always_ff @(posedge clk_i) begin : p_l15_trace
    if (rst_ni) begin
      ...
      if (l15_rtrn_i.l15_val)
        $display("[%0t] [L15ADAP] L15->RTRN type=%0d nc=%b data[63:0]=0x%016X data[127:64]=0x%016X",
                 $time, l15_rtrn_i.l15_returntype, l15_rtrn_i.l15_noncacheable,
                 l15_rtrn_i.l15_data_0, l15_rtrn_i.l15_data_1);
      // Icache fill delivered
      if (icache_rtrn_vld_o)
        $display("[%0t] [L15ADAP] IFILL->ICACHE data[63:0]=0x%016X",
                 $time, icache_rtrn_o.data[63:0]);
      // D-cache request entering FIFO (fires once when store is accepted)
      if (dcache_data_req_i && ~dcache_data_full)
        $display("[%0t] [L15ADAP] DCACHE->FIFO rtype=%0d paddr=0x%010X data=0x%016X",
                 $time, dcache_data_i.rtype, dcache_data_i.paddr, dcache_data_i.data);
    end
  end
//pragma translate_on
